# Script completo para reconstruir index.html
import re

# Ler o arquivo de backup que tem todas as seções
with open('index_backup_quebrado.html', 'r', encoding='utf-8') as f:
    backup_content = f.read()

# Extrair todas as seções (de <section até </section>)
sections_pattern = r'(<section id="[^"]+".+?</section>)'
sections = re.findall(sections_pattern, backup_content, re.DOTALL)

print(f"Encontradas {len(sections)} seções no backup")
...
# Adicionar todas as seções encontradas
for section in sections:
    html_complete += "            " + section + "\n\n"
...
print(f"✓ Arquivo index.html reconstruído com sucesso!")
print(f"✓ Total de seções incluídas: {len(sections)}")
print(f"✓ Tamanho do arquivo: {len(html_complete)} caracteres")
